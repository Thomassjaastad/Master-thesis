...
for count, strengths in enumerate(doublet_strs, 0):
    """
    # source flow island rep
    ...
    # Uncomment here for obstacles representated as doublet flow
    flow2 = Flow.Velocity(strengths, x_island0, y_island0)
    ...
    u_tot = u_source + u_doublet1 + u_doublet0 + u_sink
    v_tot = v_source + v_doublet1 + v_doublet0 + v_sink
    ...
    stream = ax[count].streamplot(X, Y, u_tot, v_tot, linewidth = 0.5, density = 2, color = 'b', 
                       arrowstyle = '->')
    ...
    ax[count].scatter(x_island1, y_island1, color = 'yellow' ,zorder = 4, label= 'source str = %.3f' % strength_island1)
    ax[count].scatter(x_island0, y_island0, color = 'orange' ,zorder = 4, label = 'source str = %.3f' % strengths)
    ax[count].scatter(x_sink, y_sink, color = 'purple', label = 'End point')
    ax[count].scatter(x_source, y_source, color = 'red', label= 'Starting point' )
